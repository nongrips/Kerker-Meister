...
var OWNERSHIP_ALPHA = 0.5:
	set(_val): set_ownership_alpha_graphics(_val)
...
func clear():
	if slabOwnershipImage.get_size().x > 0:
		...
		slabOwnershipTexture.set_image(slabOwnershipImage)

func start():
	var CODETIME_START = OS.get_ticks_msec()
	slabOwnershipImage = Image.create(M.xSize,M.ySize,false,Image.FORMAT_RGBA8)
	...
	#print(oColorRectSlabOwner.scale)
	#oColorRectSlabOwner.position = Vector2(96,96)
