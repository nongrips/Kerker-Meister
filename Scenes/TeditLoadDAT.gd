...
func start_file_selection():
	...
	var baseDir = filePath.get_base_dir().path_join("")
...
func store_original_dat_info(pathArgument: String):
	...
	var originalDatDir = pathArgument.get_base_dir().path_join("")
	var originalDatPath = pathArgument.get_base_dir().path_join("")
...
func convert_dat_to_rgb_image(datPathArgument: String) -> Image:
	...
	var rgbFullImage = Image.create(l8FullImage.get_width(), l8FullImage.get_height(), false, Image.FORMAT_RGB8)
