...
func create_research_control(parent: VBoxContainer, array_index: int, value, section_name: String, itemIndex: int, revert_button_scene, editor_context):
	...
	type_label.mouse_entered.connect(_on_research_label_mouse_entered.bind(type_label, section_name, array_index))
	type_label.mouse_exited.connect(_on_research_label_mouse_exited.bind(type_label, section_name, array_index))
	...
	kind_label.gui_input.connect(_on_kind_label_clicked.bind(array_index, kind_items, editor_context))
	kind_label.mouse_entered.connect(_on_research_label_mouse_entered.bind(kind_label, section_name, array_index))
	kind_label.mouse_exited.connect(_on_research_label_mouse_exited.bind(kind_label, section_name, array_index))
	...
	points_spinbox.value_changed.connect(_on_points_changed.bind(array_index))
	points_spinbox.get_line_edit().focus_entered.connect(editor_context._on_spinbox_focus_entered.bind(points_spinbox))
	points_spinbox.get_line_edit().focus_exited.connect(editor_context._on_spinbox_focus_exited.bind(points_spinbox, section_name, str(array_index)))
	points_spinbox.mouse_entered.connect(editor_context._on_control_mouse_entered.bind(type_label, points_spinbox, section_name, str(array_index)))
	points_spinbox.mouse_exited.connect(editor_context._on_control_mouse_exited.bind(type_label, points_spinbox, section_name, str(array_index)))
	control_container.add_child(points_spinbox)
	
	var revert_button = revert_button_scene.instantiate()
	revert_button.pressed.connect(_on_research_revert_pressed.bind(section_name, array_index))
...
func _on_research_type_selected(type_name: String, metadata: Dictionary):
	...
	oTabRules.create_selection_popup("Select Research Kind", kind_items, Callable(self, "_on_research_kind_selected"), array_index)
...
func _on_type_label_clicked(event: InputEvent, array_index: int, type_items: Array, editor_context):
	if event is InputEventMouseButton and event.pressed and event.button_index == MOUSE_BUTTON_LEFT:
		oTabRules.create_selection_popup("Select Research Type", type_items, Callable(self, "_on_research_type_selected"), array_index)
...
func _on_kind_label_clicked(event: InputEvent, array_index: int, kind_items: Array, editor_context):
	if event is InputEventMouseButton and event.pressed and event.button_index == MOUSE_BUTTON_LEFT:
		...
		oTabRules.create_selection_popup("Select Research Kind", current_kind_items, Callable(self, "_on_research_kind_selected"), array_index)
