...
#func _process(delta):
	#size = $VBoxContainer.size + Vector2(marginBorder*2,marginBorder*2)
	#$VBoxContainer.size = Vector2(0,0)
	#$VBoxContainer.position = Vector2(marginBorder,marginBorder)
...
func _on_InputsUpdateScreenCheckbox_toggled(button_pressed):
	...
	RenderingServer.render_loop_enabled = true # Just here to fix a brief 1 frame visual bug
