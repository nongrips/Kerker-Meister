...
func create_standard_config_control(parent: VBoxContainer, key: String, value, section_name: String, itemIndex: int, revert_button_scene, editor_context):
	...
	revert_button.pressed.connect(editor_context._on_revert_pressed.bind(section_name, key))
	control_container.add_child(revert_button)
	
	item_panel.mouse_entered.connect(editor_context._on_control_mouse_entered.bind(key_label, control_node, section_name, key))
	item_panel.mouse_exited.connect(editor_context._on_control_mouse_exited.bind(key_label, control_node, section_name, key))
...
func create_spinbox_control(parent: HBoxContainer, key: String, value, section_name: String, editor_context, is_float: bool = false):
	...
	spinbox.value_changed.connect(editor_context._on_value_changed.bind(section_name, key))
	spinbox.get_line_edit().focus_entered.connect(editor_context._on_spinbox_focus_entered.bind(spinbox))
	spinbox.get_line_edit().focus_exited.connect(editor_context._on_spinbox_focus_exited.bind(spinbox, section_name, key))
...
func create_line_edit_control(parent: HBoxContainer, key: String, value: String, section_name: String, editor_context):
	...
	line_edit.text_changed.connect(editor_context._on_text_changed.bind(section_name, key))
...
func create_array_control(parent: HBoxContainer, key: String, value: Array, section_name: String, editor_context):
	...
	line_edit.set_h_size_flags(Control.SIZE_EXPAND_FILL)
	editor_context.setup_script_editor_font(line_edit)
	line_edit.text_changed.connect(editor_context._on_array_text_changed.bind(section_name, key))
	parent.add_child(line_edit)
	return line_edit


func create_popup_selection(title: String, items: Array, callback: Callable, editor_context):
	var popup_selection = Window.new()
	popup_selection.title = title
	...
	item_container.add_theme_constant_override("hseparation", 20)
	item_container.add_theme_constant_override("vseparation", 2)
	...
	for item in items:
		...
		link_button.pressed.connect(editor_context._on_selection_item_pressed.bind(item))
		item_container.add_child(link_button)
	
	popup_selection.close_requested.connect(editor_context._on_popup_selection_closed)
...
func check_default_value_exists(section_name: String, array_index: int) -> bool:
	if not oCfgEditor.oConfigFileManager.default_data.has("rules.cfg"):
		return false
...
func create_selection_popup(title: String, items: Array, callback: Callable, array_index: int):
